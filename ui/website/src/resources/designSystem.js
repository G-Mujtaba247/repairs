/**
 * Modern Repairs CMS Design System
 * Color palette, typography, spacing, and component tokens
 */

export const colors = {
  // Primary brand colors
  primary: {
    50: '#f0f7ff',
    100: '#e0effe',
    200: '#bae6fd',
    300: '#7dd3fc',
    400: '#38bdf8',
    500: '#0ea5e9', // Main primary
    600: '#0284c7',
    700: '#0369a1',
    800: '#075985',
    900: '#0c3d66',
  },
  // Secondary brand colors
  secondary: {
    50: '#f5f3ff',
    100: '#ede9fe',
    200: '#ddd6fe',
    300: '#c4b5fd',
    400: '#a78bfa',
    500: '#8b5cf6', // Main secondary
    600: '#7c3aed',
    700: '#6d28d9',
    800: '#5b21b6',
    900: '#4c1d95',
  },
  // Neutral colors
  neutral: {
    50: '#f9fafb',
    100: '#f3f4f6',
    200: '#e5e7eb',
    300: '#d1d5db',
    400: '#9ca3af',
    500: '#6b7280',
    600: '#4b5563',
    700: '#374151',
    800: '#1f2937',
    900: '#111827',
  },
  // Status colors
  success: '#10b981',
  warning: '#f59e0b',
  error: '#ef4444',
  info: '#3b82f6',
  
  // Semantic
  background: '#ffffff',
  foreground: '#111827',
  card: '#f9fafb',
  border: '#e5e7eb',
  text: {
    primary: '#111827',
    secondary: '#6b7280',
    light: '#9ca3af',
  }
};

export const typography = {
  fontFamily: {
    sans: "'Inter', 'Segoe UI', Roboto, sans-serif",
    mono: "'Fira Code', monospace",
  },
  fontSize: {
    xs: '0.75rem',    // 12px
    sm: '0.875rem',   // 14px
    base: '1rem',     // 16px
    lg: '1.125rem',   // 18px
    xl: '1.25rem',    // 20px
    '2xl': '1.5rem',  // 24px
    '3xl': '1.875rem',// 30px
    '4xl': '2.25rem', // 36px
    '5xl': '3rem',    // 48px
  },
  fontWeight: {
    light: 300,
    normal: 400,
    medium: 500,
    semibold: 600,
    bold: 700,
  },
  lineHeight: {
    tight: 1.25,
    normal: 1.5,
    relaxed: 1.625,
    loose: 2,
  },
};

export const spacing = {
  xs: '0.25rem',  // 4px
  sm: '0.5rem',   // 8px
  md: '1rem',     // 16px
  lg: '1.5rem',   // 24px
  xl: '2rem',     // 32px
  '2xl': '3rem',  // 48px
  '3xl': '4rem',  // 64px
};

export const shadows = {
  xs: '0 1px 2px 0 rgba(0, 0, 0, 0.05)',
  sm: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px -1px rgba(0, 0, 0, 0.1)',
  md: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1)',
  lg: '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -4px rgba(0, 0, 0, 0.1)',
  xl: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)',
};

export const borderRadius = {
  sm: '0.375rem',   // 6px
  md: '0.5rem',     // 8px
  lg: '0.75rem',    // 12px
  xl: '1rem',       // 16px
  '2xl': '1.5rem',  // 24px
  full: '9999px',
};

export const transitions = {
  fast: '150ms cubic-bezier(0.4, 0, 0.2, 1)',
  base: '200ms cubic-bezier(0.4, 0, 0.2, 1)',
  slow: '300ms cubic-bezier(0.4, 0, 0.2, 1)',
};

// Device repair categories
export const repairCategories = [
  {
    id: 'mobile',
    name: 'Mobile Phones',
    description: 'iPhone, Android, and other smartphones',
    icon: '📱',
    issues: ['Cracked Screen', 'Battery Replacement', 'Charging Port', 'Speaker Issues', 'Water Damage'],
  },
  {
    id: 'laptop',
    name: 'Laptops & PCs',
    description: 'Desktops, laptops, and peripherals',
    icon: '💻',
    issues: ['Hardware Upgrade', 'Screen Replacement', 'Hard Drive Issues', 'Overheating', 'OS Installation'],
  },
  {
    id: 'tablet',
    name: 'Tablets',
    description: 'iPad, Android tablets, and e-readers',
    icon: '📱',
    issues: ['Screen Repair', 'Battery Issues', 'Software Problems', 'Charging Port'],
  },
  {
    id: 'appliances',
    name: 'Home Appliances',
    description: 'Refrigerators, washers, ovens, microwaves',
    icon: '🍳',
    issues: ['Not Cooling', 'Not Heating', 'Making Noise', 'Leaking Water', 'Error Codes'],
  },
];

export const repairPriorities = [
  { value: 'low', label: 'Low - Non-urgent', color: '#10b981' },
  { value: 'medium', label: 'Medium - Within 2-3 days', color: '#f59e0b' },
  { value: 'high', label: 'High - 24 hours', color: '#ef4444' },
];

export const bookingStatuses = [
  { value: 'pending', label: 'Pending', color: '#9ca3af', bgColor: '#f3f4f6' },
  { value: 'confirmed', label: 'Confirmed', color: '#3b82f6', bgColor: '#dbeafe' },
  { value: 'in_progress', label: 'In Progress', color: '#f59e0b', bgColor: '#fef3c7' },
  { value: 'completed', label: 'Completed', color: '#10b981', bgColor: '#d1fae5' },
  { value: 'cancelled', label: 'Cancelled', color: '#ef4444', bgColor: '#fee2e2' },
];

export const serviceHighlights = [
  {
    icon: '⚡',
    title: 'Fast Service',
    description: 'Same-day repairs available for most issues',
  },
  {
    icon: '✓',
    title: 'Certified Technicians',
    description: 'Factory-trained professionals with years of experience',
  },
  {
    icon: '💰',
    title: 'Affordable Pricing',
    description: 'Transparent quotes with no hidden fees',
  },
  {
    icon: '🛡️',
    title: '90-Day Warranty',
    description: 'All repairs backed by comprehensive warranty',
  },
];
